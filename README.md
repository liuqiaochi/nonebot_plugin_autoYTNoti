...
| `YT下载 <链接>` | 使用 yt-dlp 下载视频（ffmpeg 合并为 mp4）与最优封面图；封面以合并转发返回，视频随后单独直发（避免转发内视频显示「已过期」） |
...
- `YT下载 <链接>`：调用 yt-dlp 将视频与封面图下载到 `YT_DL_DIR` 目录。
  ...
  - 封面图 + 元信息以**合并转发**形式回传；视频则**紧随其后单独直发**（不放入转发节点），避免 go-cqhttp 转发内视频显示「已过期」。视频直发失败时会提示本地保存路径。
...
- `YT下载` 结果以合并转发返回封面图+元信息，视频随后单独直发（避免转发内视频显示「已过期」），发送失败时提示本地路径
